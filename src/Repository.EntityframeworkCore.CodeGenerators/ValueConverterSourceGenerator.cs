...
";
        ...
        private void Generate(GeneratorExecutionContext context, SemanticModel semanticModel,
            TypeDeclarationSyntax classDef, string rootNamespace)
        {
            ...
            var stronglyTypedId = namedTypeSymbol.AllInterfaces
                .SingleOrDefault(t => t.Name != "IStronglyTypedId" && t.Name.EndsWith("StronglyTypedId"));
            if (stronglyTypedId == null) return;
...
namespace {rootNamespace}.ValueConverters
{{
    public partial class {className}ValueConverter : ValueConverter<{className}, {stronglyTypedId.AllInterfaces.First(t => t.Name == "IStronglyTypedId").TypeArguments[0].Name}>
    {{
        public  {className}ValueConverter() : base(p => p.Id, p => new {className}(p)) {{ }}
    }}
}}
