    ...
    public class AppDbContextBaseSourceGenerator : ISourceGenerator
    {
            ...
            List<INamedTypeSymbol> ids, string rootNamespace)
        {
            ...
            foreach (var id in ids)
            {
                var idName = id.Name;
                sb.AppendLine(
                    $@"            configurationBuilder.Properties<global::{id.ContainingNamespace}.{id.Name}>().HaveConversion<{idName}ValueConverter>();");
            }
...
");
            context.AddSource($"{dbContextType.Name}ValueConverters.g.cs", source.ToString());
        }
