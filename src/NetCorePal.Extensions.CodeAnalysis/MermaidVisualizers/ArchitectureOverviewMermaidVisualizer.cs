...
public static class ArchitectureOverviewMermaidVisualizer
{
    ...
    public static string GenerateMermaid(CodeFlowAnalysisResult analysisResult)
    {
        ...
        {
            var nodeName = MermaidVisualizerHelper.SanitizeClassName(node.FullName);
            ...
            switch (node.Type)
            {
                case NodeType.Controller:
                    ...
                    nodeTypes[node.FullName] = nodeName;
                    break;
        ...
        foreach (var rel in analysisResult.Relationships)
        {
            ...
            // 使用清理后的节点全名
            if (!nodeTypes.TryGetValue(rel.FromNode.FullName, out var sourceName) ||
                !nodeTypes.TryGetValue(rel.ToNode.FullName, out var targetName)) continue;
